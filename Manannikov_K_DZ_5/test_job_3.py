...
def tutur_klas(tutor, klas):
    klas.extend(((len(tutor) - len(klas)) * [None]))  # добавляем недостоющие 'None'
    item = zip(tutor, klas)  # zipуем списки
    for zaq in item:  # генератор кортежа вида (<tutor>, <klass>)
        yield zaq


zaqzaq = tutur_klas(tutors, klasses)
